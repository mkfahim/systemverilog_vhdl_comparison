...
always @(*)//   this works
//always @(pc or rst)//// this works like above
begin
int int_;
int_ = pc;
	if (rst)
...
//		alu_logic <= #40 0;
		...
		alu_logic <= repeat (2) @(posedge clk) 0;
		load_logic <= 0;
		store_logic <= 0;
		branch_logic <= 0; end
...
//		load_logic <= #60 0;	
		...
		load_logic <= repeat (3) @(posedge clk) 0;		
		alu_logic <= 0;
		store_logic <= 0;
		branch_logic <= 0;end 
...
//		store_logic <= #80 0;	
		...
		store_logic <= repeat (4) @(posedge clk) 0;
		alu_logic <= 0;
		load_logic <= 0;
		branch_logic <= 0; end
...
//		branch_logic <= #100 0;			
		...
		branch_logic <= repeat (5) @(posedge clk) 0;	
		alu_logic <= 0;
		load_logic <= 0;
		store_logic <= 0;end
